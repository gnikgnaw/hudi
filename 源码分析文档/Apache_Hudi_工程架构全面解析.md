...
Hudi 采用经典的**分层架构 + 多模块 Maven 工程**，共计 30+ 个 Maven 模块。按职责可分为以下六层：
...
│  hudi-aws │ hudi-gcp │ hudi-trino-plugin                       │
└─────────────────────────────────────────────────────────────────┘
```
...
│   ├── client/
│   │   ├── BaseHoodieWriteClient.java       # 核心写入客户端（84KB 大文件）
...
| `hudi-trino-plugin` | Trino 查询引擎插件 | 让 Trino 直接读取 Hudi 表 |
...
Hudi 的索引体系是其**高效 upsert 能力**的关键：

```
HoodieIndex (抽象基类)
├── BLOOM           # 布隆过滤器索引（分区内唯一）
├── GLOBAL_BLOOM    # 全局布隆过滤器索引
├── SIMPLE          # 简单 Join 索引（分区内唯一）
├── GLOBAL_SIMPLE   # 全局简单索引
├── BUCKET          # 桶索引（哈希分桶）
│   ├── SIMPLE Engine           # 固定桶数
│   └── CONSISTENT_HASHING     # 一致性哈希，支持动态扩缩容
├── INMEMORY        # 内存索引
├── FLINK_STATE     # Flink 状态后端索引
├── GLOBAL_RECORD_LEVEL_INDEX   # 全局记录级索引（基于 Metadata Table）
...
- **RECORD_LEVEL_INDEX**: 利用 Metadata Table 存储精确映射，最精确但空间开销更大
...
├── FILES                  # 文件列表索引（替代 fs.list）
├── COLUMN_STATS           # 列统计信息（min/max/count/null count）
├── BLOOM_FILTERS          # 布隆过滤器
├── RECORD_INDEX           # 记录级索引
├── SECONDARY_INDEX        # 二级索引
├── EXPRESSION_INDEX       # 表达式索引
├── PARTITION_STATS        # 分区级统计信息
└── ...
```
...
Hudi 在 `packaging/` 下维护了 17 个 bundle 模块，按用途可分为以下几类：
...
| **其他** | `hudi-integ-test-bundle` | 集成测试 |
| | `hudi-timeline-server-bundle` | Timeline Server 独立部署 |
...
#### 8.5.3 SparkAdapter Trait：40+ 个抽象方法
