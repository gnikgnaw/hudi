...
Hudi 采用经典的**分层架构 + 多模块 Maven 工程**，根 pom.xml 中定义了 26 个顶级模块（包含子模块共计 50+ 个）。按职责可分为以下六层：
...
├── BLOOM_FILTERS          # 布隆过滤器
├── RECORD_INDEX           # 记录级索引
├── EXPRESSION_INDEX       # 表达式索引（前缀 expr-index-）
├── SECONDARY_INDEX        # 二级索引（前缀 secondary-index-）
...
Hudi 在 `packaging/` 下维护了 16 个 bundle 模块（含 1 个验证工具目录），按用途可分为以下几类：
