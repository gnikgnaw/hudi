...
小文件问题是数据湖最普遍的性能杀手,核心影响包括:

1. **查询性能急剧下降**: 查询引擎(Spark/Presto/Trino)需要为每个文件打开一个文件句柄,小文件越多,文件打开开销越大。例如 1 万个小文件的查询延迟可能是 100 个大文件的 10-100 倍。

2. **NameNode 压力**: HDFS NameNode 需要为每个文件维护元数据(约 150 字节/文件),1 亿个小文件会消耗约 15GB NameNode 内存。

3. **调度开销**: Spark 等引擎为每个文件分配 Task,小文件过多导致 Task 数量爆炸,调度器成为瓶颈。

4. **存储效率低**: Parquet 等列式格式的压缩和编码在大文件上效果更好,小文件无法充分利用列式存储优势。

**实际场景举例**:
- 流式写入场景: Flink 每 1 分钟 checkpoint 一次,每次写入 1000 条记录,一天产生 1440 个小文件
- 高基数分区: 按 user_id 分区,100 万用户每人只有几条记录,产生 100 万个微小文件
- 数据倾斜: 大部分 FileGroup 正常,少数 FileGroup 因 UPDATE 频繁产生大量小 log 文件

#### 2.0.2 有什么坑

**坑 1: 小文件阈值设置不当**
- `hoodie.parquet.small.file.limit` 设置过大(如接近 `max.file.size`),会导致写入时频繁尝试填充,反而降低写入吞吐
- 设置为 0 会完全关闭小文件填充,适合 bulk_insert 但不适合流式写入
- **源码证据**: `UpsertPartitioner.getSmallFiles()` (L295-318) 会扫描所有小于阈值的文件,阈值过大导致扫描开销增加

**坑 2: 平均记录大小估算失效**
- `UpsertPartitioner.assignInserts()` (L176-178) 使用历史 commit 的平均记录大小估算,但排除了 `REPLACE_COMMIT_ACTION`(Clustering 产生)
- **为什么?** Clustering 后的文件记录密度更高(排序+压缩),会低估实际记录大小,导致新文件超出 `max.file.size`
- **源码证据**: 
```java
// L176-178: 只用 COMMIT/DELTA_COMMIT/REPLACE_COMMIT 计算平均大小
...
```
- 注释明确说明: "we only use commit instants to calculate average record size because replacecommit can be created by clustering, which has smaller average record size"

**坑 3: 与 Clustering 冲突**
- 如果小文件正在被 Clustering 处理,同时又被 UpsertPartitioner 选中填充,会导致写入冲突
- **源码证据**: `UpsertPartitioner.filterSmallFilesInClustering()` (L158-165) 专门过滤掉 pending clustering 的文件
```java
// L159-161: 排除正在 Clustering 的文件
return smallFiles.stream()
    .filter(smallFile -> !pendingClusteringFileGroupsId.contains(smallFile.location.getFileId()))
    .collect(Collectors.toList());
```

**坑 4: 首次写入时无历史数据**
- 表的第一次写入时,`commitTimeline.empty()` 为 true,`getSmallFiles()` 返回空列表,无法填充
- **源码证据**: `UpsertPartitioner.getSmallFiles()` (L300-302) 检查 `commitTimeline.empty()`

#### 2.0.3 核心概念解释

**SmallFile**: 
- 定义: 文件大小 < `hoodie.parquet.small.file.limit` 的 Base File
- 数据结构: `SmallFile` 类包含 `HoodieRecordLocation location` 和 `long sizeBytes` 两个字段
- **源码位置**: `UpsertPartitioner` 内部类 (L68)

**小文件填充 (Small File Handling)**:
- 机制: INSERT 记录优先路由到已有的小文件对应的 bucket,而不是创建新文件
- 计算公式: `recordsToAppend = (maxFileSize - smallFile.sizeBytes) / averageRecordSize`
- **源码证据**: `UpsertPartitioner.assignInserts()` (L206-229)

**Bucket 分配策略**:
- UPDATE bucket: 每个 FileGroup 一个 bucket,直接映射
- INSERT bucket: 按权重分配,权重 = 该 bucket 容量 / 总 INSERT 数
- **源码证据**: `UpsertPartitioner.getPartition()` (L344-371) 使用 MD5 hash + 二分查找实现权重路由

#### 2.0.4 设计理念

**分层治理而非单一方案**:
Hudi 采用"预防 + 修复 + 回收"三层体系,而不是依赖单一机制:
- Layer 1 (UpsertPartitioner): 写入时预防,成本最低
- Layer 2 (Clustering): 事后修复,成本中等
- Layer 3 (Clean): 空间回收,成本最低

**自适应而非硬编码**:
- 平均记录大小通过历史统计自动计算,不需要用户手动配置
- Bucket 数量根据 INSERT 记录数和目标文件大小动态计算
- **源码证据**: `UpsertPartitioner.assignInserts()` (L234-236) 的自适应逻辑
```java
...
**协调而非冲突**:
- 通过 `getPartitionPathToPendingClusteringFileGroupsId()` 获取正在 Clustering 的文件,避免同时操作
- 这体现了 Hudi 表服务间的冲突协调机制
- **源码证据**: `UpsertPartitioner.assignInserts()` (L184-193)

**权衡写入吞吐与文件大小**:
- 小文件填充会增加写入路径的计算开销(扫描小文件 + 计算容量)
- 但避免了后续 Clustering 的重写成本
- 通过 `small.file.limit = 0` 可以完全关闭填充,适合 bulk_insert 场景
...
Compaction 专门解决 MOR 表的**读放大问题**:

1. **Log 文件堆积导致读取性能线性下降**: MOR 表每次 UPDATE/DELETE 产生 log 文件,读取时需要合并 base + 所有 log,log 越多读取越慢。例如 10 个 log 文件的读取时间可能是只读 base 的 5-10 倍。

2. **内存压力**: 合并 log 时需要构建 `ExternalSpillableMap` 存储所有 log records,log 过多会导致内存溢出或频繁 spill 到磁盘。

3. **CPU 开销**: 遍历 base records 时需要逐一检查是否有 log 更新,log 越多检查开销越大。

4. **查询引擎兼容性**: 某些查询引擎(如 Presto 早期版本)不支持 MOR 读取,必须通过 Compaction 转换为 COW 格式。

**如果没有 Compaction**:
- MOR 表的读取性能会随着写入次数线性下降,最终变得不可用
- 存储空间持续增长(log 文件累积),无法回收旧版本数据

**实际场景举例**:
- 实时数仓场景: Flink 每分钟写入一次,一天产生 1440 个 delta commit,每个 FileGroup 有 1440 个 log 文件,读取时需要合并所有 log
- CDC 场景: 上游数据库频繁 UPDATE,MOR 表的 log 文件快速堆积

#### 3.0.2 有什么坑

**坑 1: Compaction 积压导致雪崩**
- 如果 Compaction 速度 < 写入产生 log 的速度,pending compaction 会越来越多,最终导致读取性能崩溃
- **诊断方法**: 检查 `.hoodie/` 目录中 `.compaction.requested` 文件数量
- **源码证据**: `BaseHoodieCompactionPlanGenerator.generateCompactionPlan()` 会为每个需要 Compaction 的 FileSlice 生成一个 `CompactionOperation`,如果积压严重,plan 文件会越来越多

**坑 2: Compaction 与写入冲突**
- Compaction 执行期间,该 FileGroup 不能接受新的 UPDATE(会导致数据不一致)
- **源码证据**: `BaseHoodieCompactionPlanGenerator` (L683) 构建排除集合 `fgIdsInPendingCompactionAndClustering`,写入时会跳过这些 FileGroup
```java
// L683: 排除正在 Compaction 的 FileGroup
Set<HoodieFileGroupId> fgIdsInPendingCompactionAndClustering = ...;
```

**坑 3: Log 文件时间安全问题**
- 如果 Compaction plan 包含了尚未完成的 delta commit 的 log 文件,会导致数据重复读取
- **源码证据**: `BaseHoodieCompactionPlanGenerator` (L713-720) 专门过滤 pending log 文件
```java
// L713-720: 只保留 completionTime < compactionInstant 的 log 文件
s.filterLogFiles(logFile ->
    completionTimeQueryView.isCompletedBefore(compactionInstant, logFile.getDeltaCommitTime()));
```
- 注释解释: "假设 t10 发起 delta commit, t20 调度 compaction (此时 t10 还在 inflight), 如果将 t10 的 log 文件包含进 compaction plan, 当 t30 执行 compaction 时 t10 已完成,reader 会认为该 log 有效,但 compaction 也合并了它 → 数据被重复读取!"

**坑 4: Compaction 策略选择不当**
- `LogFileSizeBasedCompactionStrategy` 按 log 大小降序排序,大的优先 Compaction
- 如果设置 `hoodie.compaction.logfile.size.threshold` 过大,会导致很多 FileSlice 被过滤掉,无法及时 Compaction
- **源码证据**: `LogFileSizeBasedCompactionStrategy.orderAndFilter()` (L50-56) 过滤 log 总大小 < threshold 的 FileSlice

**坑 5: Inline Compaction 阻塞写入**
- `hoodie.compact.inline=true` 会在写入路径中同步执行 Compaction,导致写入延迟 = 写入时间 + Compaction 时间
- 生产环境应使用 Async 或 Separate Job 模式
- **源码证据**: `HoodieCompactor.compact()` (L84-148) 是阻塞执行的

#### 3.0.3 核心概念解释

**Compaction vs Log Compaction**:
- **Compaction**: 合并 base + log → 新 base (普通 Compaction)
- **Log Compaction**: 合并多个小 log → 更少的大 log (轻量级 Compaction)
- **源码证据**: `HoodieCompactor.compact()` (L132-147) 根据 `operationType` 选择不同的执行路径
```java
if (operationType == WriteOperationType.LOG_COMPACT) {
    return context.parallelize(operations).map(
        operation -> logCompact(...))  // 使用 FileGroupReaderBasedAppendHandle
        .flatMap(List::iterator);
} else {
    return context.parallelize(operations).map(
        operation -> compact(...))  // 使用 HoodieMergeHandle
        .flatMap(List::iterator);
}
```

**CompactionPlan**:
- 定义: Compaction 的执行计划,包含所有需要 Compaction 的 FileSlice 信息
- 存储格式: Avro 序列化,存储在 `.hoodie/T.compaction.requested` 文件中
- **源码位置**: `org.apache.hudi.avro.model.HoodieCompactionPlan`

**CompactionOperation**:
- 定义: 单个 FileSlice 的 Compaction 操作,包含 base file + log files + metrics
- **源码证据**: `HoodieCompactor.compact()` (L121-122) 将 Avro 格式的 operation 转换为 POJO
```java
...
**两阶段设计**:
- **Schedule**: 生成 CompactionPlan,标记为 REQUESTED 状态
- **Execute**: 执行 Compaction,REQUESTED → INFLIGHT → COMPLETED
- **源码证据**: `HoodieCompactor.compact()` (L93-102) 执行状态转换
```java
// L93-102: 状态转换 REQUESTED → INFLIGHT
...
1. **跨引擎执行**: Flink 写入数据,Spark 执行 Compaction。Schedule 产生 JSON 格式的 plan,任何引擎都能读取和执行。
   - **源码证据**: `CompactionPlan` 使用 Avro 序列化,与引擎无关

2. **异步解耦**: Schedule 在写入路径中执行(轻量),Execute 可以异步/独立执行(重量)。写入不被 Compaction 阻塞。
   - **源码证据**: `HoodieCompactor` 是抽象基类,Spark/Flink/Java 各有实现,支持不同的执行模式

3. **故障恢复**: 如果 Execute 失败,CompactionPlan 仍然存在(REQUESTED 状态),可以直接重试,不需要重新 Schedule。
   - **源码证据**: Flink 的 `HoodieFlinkMergeOnReadTableCompactor.preCompact()` 会检查 INFLIGHT 状态并回滚,支持 Checkpoint 恢复

**引擎差异化适配**:
Hudi 使用模板方法模式,基类定义核心流程,子类实现引擎特定逻辑:
- **Spark**: `maybePersist()` 调用 `writeStatus.persist()` 缓存中间结果
- **Flink**: `maybePersist()` 是空操作,因为流式执行不需要缓存
- **Java**: 不支持 Log Compaction
- **源码证据**: `HoodieCompactor` (L62-80) 定义三个抽象方法,由子类实现

**贪心策略优化收益**:
`LogFileSizeBasedCompactionStrategy` 按 log 大小降序排序,大的优先 Compaction。为什么?
- log 越大的 FileSlice,读放大越严重,Compaction 的收益越高
- 每次 Compaction 的资源有限(受 `target.io` 限制),应该优先处理收益最大的
- **源码证据**: `LogFileSizeBasedCompactionStrategy.compare()` (L70-75) 实现降序比较
```java
...
**I/O 限制避免资源耗尽**:
`BoundedIOCompactionStrategy` 限制单次 Compaction 的总 I/O(读 + 写),避免 Compaction 作业占用过多资源影响写入。
- **源码证据**: `BoundedIOCompactionStrategy.orderAndFilter()` (L39-60) 累加 I/O 直到超过 `targetIORemaining`
```java
...
Clustering 解决 Compaction 无法解决的**文件布局优化问题**:

1. **跨 FileGroup 的小文件合并**: Compaction 只能合并单个 FileGroup 内的 base + log,无法合并多个小 FileGroup。Clustering 可以将多个小 FileGroup 合并为更少的大 FileGroup。

2. **数据局部性优化**: 按查询常用字段排序,使相近的数据聚集在同一文件中,提升 Data Skipping 效果。例如按 `city` 排序后,查询 `WHERE city='Beijing'` 只需读取少数文件。

3. **文件大小均匀化**: 调整文件大小到目标值(如 128MB),避免文件大小不均导致的查询倾斜。

4. **Z-Order/Hilbert 多维排序**: 支持多列组合查询的优化,比单列排序更适合复杂查询场景。

**如果没有 Clustering**:
- 小文件会持续累积,即使 Compaction 也无法解决(因为 Compaction 不跨 FileGroup)
- 查询性能随着数据增长持续下降,Data Skipping 效果差
- 文件大小不均导致查询倾斜,部分 Task 执行时间远超其他 Task

**实际场景举例**:
- 高基数分区场景: 按 user_id 分区,每个分区有大量小 FileGroup,Clustering 可以合并为少数大文件
- 多维查询场景: 查询同时过滤 `city` 和 `age`,使用 Z-Order 排序可以同时优化两个维度的 Data Skipping

#### 4.0.2 有什么坑

**坑 1: Clustering 与写入冲突**
- Clustering 会替换整个 FileGroup(旧 FileGroup 被标记为 replaced),如果同时有写入操作,会导致冲突
- **源码证据**: `BaseHoodieCompactionPlanGenerator` (L686) 排除 pending clustering 的 FileGroup,避免 Compaction 与 Clustering 冲突

**坑 2: 排序列选择不当**
- 如果 `sort.columns` 选择了高基数列(如 user_id),排序效果差,Data Skipping 收益低
- 应选择查询最频繁的过滤列,且基数适中(如 city, date)
- **源码证据**: `ClusteringExecutionStrategy` 使用 `RowCustomColumnsSortPartitioner` 或 `RowSpatialCurveSortPartitioner` 实现排序

**坑 3: 目标文件大小设置不当**
- `hoodie.clustering.plan.strategy.target.file.max.bytes` 设置过小,会产生过多文件
- 设置过大,会导致单个文件过大,查询时无法并行
- 推荐值: 128MB - 256MB

**坑 4: Clustering 频率过高**
- `hoodie.clustering.inline.max.commits` 设置过小,会导致频繁 Clustering,影响写入吞吐
- 推荐值: 4-10,根据写入频率调整

**坑 5: 与 Savepoint 冲突**
- Clustering 会替换旧 FileGroup,如果旧 FileGroup 被 Savepoint 引用,Clean 时不能删除
- **源码证据**: `CleanPlanner.isFileSliceExistInSavepointedFiles()` (L337-347) 检查 FileSlice 是否被 Savepoint 引用

#### 4.0.3 核心概念解释

**ClusteringPlan vs ClusteringGroup**:
- **ClusteringPlan**: 整个 Clustering 操作的计划,包含多个 ClusteringGroup
- **ClusteringGroup**: 一组需要一起重写的 FileSlice,Clustering 时会读取这些 FileSlice 的数据,排序后写入新文件
- **源码证据**: `ClusteringExecutionStrategy.performClustering()` (L90) 接收 `HoodieClusteringPlan` 参数

**ReplaceCommit vs Commit**:
- Clustering 使用 `replacecommit` 而不是 `commit`,因为它会替换旧 FileGroup 为新 FileGroup
- **源码证据**: Timeline 中 Clustering 产生的 instant 类型为 `REPLACE_COMMIT_ACTION`

**Data Skipping**:
- 定义: 查询引擎根据文件的 min/max 统计信息跳过不相关的文件
- Clustering 通过排序使文件的 min/max 范围更窄,提升 Data Skipping 效果
- 例如: 排序前 file1 的 city 范围是 [Anhui, Zhejiang],排序后是 [Anhui, Chongqing],范围更窄

**Z-Order vs Hilbert Curve**:
- **Z-Order**: 将多维坐标交错编码为一维值,保持多维局部性
- **Hilbert Curve**: 比 Z-Order 更复杂的空间填充曲线,局部性更好,但计算开销更大
- **源码证据**: `RowSpatialCurveSortPartitioner` 支持 Z-Order 和 Hilbert 两种模式

#### 4.0.4 设计理念

**使用 BulkInsert 而非普通 Write**:
Clustering 使用 `BulkInsert` 写入新文件,而不是普通的 `Upsert/Insert`。为什么?
- BulkInsert 跳过了 UpsertPartitioner 的小文件检测和 FileGroup 匹配逻辑,直接创建新文件
- Clustering 的目标就是重写一组全新的文件,不需要与已有文件关联
- **源码证据**: `SparkSortAndSizeExecutionStrategy` 使用 `HoodieDatasetBulkInsertHelper.bulkInsert()` 写入

**条件优化 + 优雅降级**:
`SparkBinaryCopyClusteringExecutionStrategy` 在满足条件时直接在二进制层面拷贝 Parquet 文件的 Row Group,而不需要反序列化再序列化。条件不满足时自动降级为常规逻辑。
- **启用条件**: COW 表 + 无排序列 + Parquet 格式 + Schema 兼容
- **源码证据**: `SparkBinaryCopyClusteringExecutionStrategy` 继承自 `SparkSortAndSizeExecutionStrategy`,条件不满足时调用父类方法

**MultipleSparkJobExecutionStrategy vs SingleSparkJobExecutionStrategy**:
- **MultipleSparkJobExecutionStrategy**: 为每个 ClusteringGroup 提交一个独立的 CompletableFuture,并行执行,支持独立失败和重试
- **SingleSparkJobExecutionStrategy**: 将所有 ClusteringGroup 用一个 `parallelize().map()` 处理,适合 ClusteringGroup 数量非常大的场景,避免过多 Job 带来的调度开销
- **源码证据**: `MultipleSparkJobExecutionStrategy` 和 `SingleSparkJobExecutionStrategy` 是 `ClusteringExecutionStrategy` 的两个子类

**Flink 的特殊实现**:
Flink 引擎没有使用 `ClusteringExecutionStrategy` 继承体系,而是在 `ClusteringOperator` 中直接实现了 Clustering 执行逻辑。这是因为 Flink 的流式执行模型要求 Clustering 操作内嵌在 Flink DAG 中,而不是像 Spark 那样以独立 Job 形式存在。
- **源码位置**: `hudi-flink-datasource/hudi-flink/src/main/java/org/apache/hudi/sink/clustering/ClusteringOperator.java`
...
1. **过期文件累积**: COW 表每次 UPDATE 产生新版本文件,MOR 表 Compaction 后产生新 base 文件,旧文件不会自动删除,导致存储空间持续增长。

2. **时间旅行查询支持**: 需要保留一定时间窗口内的历史版本,支持时间旅行查询和增量消费,但不能无限保留。

3. **Savepoint 保护**: 某些关键时间点的数据需要长期保留(如数据迁移前的快照),Clean 时不能删除。

4. **与表服务协调**: Clean 需要避免删除正在被 Compaction/Clustering 使用的文件。

**如果没有 Clean**:
- 存储成本无限增长,100 个 FileGroup × 100 个版本 = 10000 个文件
- 文件系统元数据压力增大(HDFS NameNode 内存占用)
- 查询性能下降(需要扫描更多文件)

**实际场景举例**:
- 高频更新场景: 每小时更新一次,一天产生 24 个版本,一个月产生 720 个版本,不 Clean 会导致存储成本爆炸
- 合规要求场景: 数据保留 72 小时,超过 72 小时的数据必须删除

#### 5.0.2 有什么坑

**坑 1: Clean 策略与增量消费不匹配**
- 如果增量消费延迟大于 Clean 保留窗口,会导致增量消费丢数据
- 例如: `KEEP_LATEST_COMMITS=10`,但增量消费延迟 15 个 commit,会丢失 5 个 commit 的数据
- **解决方案**: 设置 `commits.retained` ≥ 增量消费延迟的 commit 数

**坑 2: Savepoint 被误删**
- Clean 必须检查 Savepoint 引用的文件,避免误删
- **源码证据**: `CleanPlanner.isFileSliceExistInSavepointedFiles()` (L337-347) 检查 FileSlice 是否被 Savepoint 引用
```java
...
**坑 3: 增量 Clean 模式下 Savepoint 删除导致降级**
- 如果上次 Clean 以来有 Savepoint 被删除,增量 Clean 必须降级为全量 Clean
- **为什么?** Savepoint 的删除意味着之前受保护的文件现在可能可以被清理了,而增量模式可能不会扫描到这些文件所在的分区
- **源码证据**: `CleanPlanner.isAnySavepointDeleted()` (L246-257) 检查 Savepoint 是否被删除
```java
...
**坑 4: Pending Compaction 文件被误删**
- Clean 必须检查文件是否被 pending compaction 引用,避免误删
- **源码证据**: `CleanPlanner.isFileSliceNeededForPendingMajorOrMinorCompaction()` (L643-645) 检查 FileSlice 是否被 pending compaction 引用

**坑 5: MOR Metadata Table 的特殊优化失效**
- MOR Metadata Table 在只有 delta commit 的情况下可以跳过 Clean(因为没有旧版本需要清理)
- 但如果有 commit(Compaction 产生),必须执行 Clean
- **源码证据**: `CleanPlanner.canSkipClean()` (L173-190) 检查是否可以跳过 Clean

#### 5.0.3 核心概念解释

**三种 Clean 策略**:
- **KEEP_LATEST_COMMITS**: 保留最近 N 次 commit 的文件,适合增量消费场景
- **KEEP_LATEST_FILE_VERSIONS**: 保留每个文件的最近 N 个版本,适合时间旅行场景
- **KEEP_LATEST_BY_HOURS**: 保留最近 N 小时内的文件,适合合规要求场景
- **源码位置**: `org.apache.hudi.common.model.HoodieCleaningPolicy`

**earliestCommitToRetain**:
- 定义: Clean 时保留的最早 commit,早于此 commit 的文件可以被删除
- 计算方式: 根据 Clean 策略和配置参数计算
- **源码证据**: `CleanPlanner.getEarliestCommitToRetain()` (L618-629) 计算 earliestCommitToRetain

**增量 Clean vs 全量 Clean**:
- **增量 Clean**: 只扫描上次 Clean 以来有变更的分区,适合大表
- **全量 Clean**: 扫描所有分区,适合小表或首次 Clean
- **源码证据**: `CleanPlanner.getPartitionPathsForIncrementalCleaning()` (L224-244) 实现增量 Clean

**Savepoint 保护机制**:
- Clean 时会收集所有 Savepoint 引用的文件,这些文件不会被删除
- **源码证据**: `CleanPlanner` 构造函数 (L99-117) 收集 `savepointedTimestamps`

#### 5.0.4 设计理念

**安全第一**:
Clean 的设计原则是"宁可不删,不可误删"。多重保护机制确保不会删除正在被使用的文件:
1. **Savepoint 保护**: 检查文件是否被 Savepoint 引用
2. **Pending Compaction 保护**: 检查文件是否被 pending compaction 引用
3. **Inflight 保护**: 只清理 `earliestInflightInstant` 之前的文件
4. **时间窗口保护**: 根据 Clean 策略保留一定时间窗口内的文件

**源码证据**: `CleanPlanner.getFilesToCleanKeepingLatestVersions()` (L354-405) 和 `getFilesToCleanKeepingLatestCommits()` (L428-507) 都实现了 Savepoint 保护

**增量优化**:
对于大表(数万个分区),全量 Clean 需要扫描所有分区,耗时很长。增量 Clean 只扫描上次 Clean 以来有变更的分区,大幅降低 Clean 耗时。
- **源码证据**: `CleanPlanner.getPartitionPathsForIncrementalCleaning()` (L224-244) 实现增量 Clean
```java
...
**策略灵活性**:
三种 Clean 策略适应不同场景:
- 增量消费场景 → `KEEP_LATEST_COMMITS`,保证增量消费不丢数据
- 时间旅行场景 → `KEEP_LATEST_FILE_VERSIONS`,每个文件版本数固定
- 合规场景 → `KEEP_LATEST_BY_HOURS`,精确的时间窗口

**与表服务协调**:
Clean 需要与 Compaction/Clustering 协调,避免删除正在被使用的文件:
- **源码证据**: `CleanPlanner` 构造函数 (L104-112) 收集 `fgIdToPendingCompactionOperations` 和 `fgIdToPendingLogCompactionOperations`
```java
...
Archival 解决**Timeline 文件过多导致的初始化性能问题**:

1. **MetaClient 初始化慢**: 每次初始化 `HoodieTableMetaClient` 需要 LIST `.hoodie/` 目录,Timeline 文件越多,LIST 操作越慢。例如 30000 个文件在 S3 上需要 30 次 LIST API 调用(每页 1000 个)。

2. **Timeline 加载慢**: 需要读取所有 Timeline 文件构建 `HoodieTimeline`,文件越多加载越慢。

3. **文件系统压力**: 大量小文件(每个 instant 3 个文件: requested/inflight/completed)对文件系统元数据造成压力。

4. **查询延迟**: 每次查询都需要初始化 MetaClient,Timeline 文件过多会显著增加查询延迟。

**如果没有 Archival**:
- 写入 10000 次 → 30000 个 Timeline 文件 → MetaClient 初始化耗时数秒甚至数十秒
- 长期运行的表(数年)可能有数百万个 Timeline 文件,完全不可用

**实际场景举例**:
- 高频写入场景: Flink 每分钟写入一次,一天产生 1440 个 commit,一年产生 52 万个 commit,对应 156 万个 Timeline 文件
- 在 S3 上,156 万个文件的 LIST 操作需要 1560 次 API 调用,耗时数分钟

#### 6.0.2 有什么坑

**坑 1: Archival 与 Clean 配置不匹配**
- `hoodie.keep.min.commits` 必须 > `hoodie.cleaner.commits.retained`,否则 Clean 可能引用已归档的 instant
- **为什么?** Clean 需要读取历史 commit 的元数据来决定哪些文件可以删除,如果这些 commit 已被归档,Clean 会失败
- **源码证据**: Archival 的五重保护规则之一是"不影响 Compaction 调度",确保 Compaction 需要的最老 delta commit 不被归档

**坑 2: Archival 与增量消费不匹配**
- 如果增量消费延迟大于 Archival 保留窗口,会导致增量消费失败(无法读取已归档的 commit 元数据)
- **解决方案**: 增大 `hoodie.keep.min.commits`,确保 ≥ 增量消费延迟的 commit 数

**坑 3: Savepoint 保护失效**
- `shouldArchiveBeyondSavepoint=false` 时,Archival 会在第一个 Savepoint 处停止归档,导致 Savepoint 之前的 commit 永远不会被归档
- `shouldArchiveBeyondSavepoint=true` 时,会跳过 Savepoint 对应的 commit 继续归档,但 Savepoint 本身不会被归档
- **源码证据**: Archival 的六重保护规则包含 Savepoint 保护

**坑 4: Metadata Table Compaction 限制**
- Archival 不能归档 Metadata Table 最新 compaction 时间之后的 commit,否则 MDT 无法通过 replay 构建元数据
- **源码证据**: Archival 的五重保护规则之一是"不超过 MDT Compaction"

**坑 5: V1 vs V2 性能差异**
- V1 使用 Avro Log 追加写,归档数据量大时读取性能差(需要顺序扫描整个 Log)
- V2 使用 LSM-Tree + Parquet,读取性能好,但写入逻辑更复杂
- **源码证据**: `TimelineArchiverV1` 和 `TimelineArchiverV2` 是两个不同的实现

#### 6.0.3 核心概念解释

**Active Timeline vs Archived Timeline**:
- **Active Timeline**: `.hoodie/` 目录中的 Timeline 文件,MetaClient 初始化时加载
- **Archived Timeline**: `.hoodie/archived/` 目录中的归档文件,只在需要时加载(如时间旅行查询)
- **源码位置**: `HoodieActiveTimeline` 和 `HoodieArchivedTimeline`

**Archival V1 vs V2**:
- **V1**: 使用 Avro Log 文件格式,追加写,永久禁用合并
- **V2**: 使用 LSM-Tree + Parquet 文件格式,支持 Compaction 和 Cleaning
- **源码位置**: `TimelineArchiverV1` 和 `TimelineArchiverV2`

**LSM-Tree 文件命名规则**:
- 格式: `${min_instant}_${max_instant}_${level}.parquet`
- `min_instant` / `max_instant`: 文件包含的 Instant 时间范围
- `level`: LSM 层级(0 为最新层,数字越大越老)
- **源码证据**: V2 的归档文件使用此命名规则

**Manifest + Version 文件**:
- V2 使用 Manifest + Version 文件来管理 LSM-Tree 的一致性视图
- Manifest 记录所有归档文件的列表,Version 文件指向最新的 Manifest
- **源码证据**: `LSMTimelineWriter.compactAndClean()` (L255-270) 更新 Manifest
...
**源码证据**: 文档第 10.6 节详细描述了这些保护规则

**从 V1 演进到 V2 的原因**:
V1 的 Avro Log 追加写模式在归档数据量大时(如运行数年的表,可能有数百万个归档 Instant)面临严重的读取性能问题——每次读取归档 Timeline 都需要反序列化整个 Log 文件。V2 的 LSM-Tree + Parquet 方案解决了这个问题:
- Parquet 的列式存储和 Predicate Pushdown 使得范围查询归档 Timeline 非常高效
- LSM-Tree 的分层合并避免了小文件过多
- Manifest 机制提供了原子性的视图管理,支持并发归档操作

**源码证据**: 文档第 10.6 节详细对比了 V1 和 V2 的差异

**LSM-Tree Compaction 机制**:
V2 的 Compaction 触发条件: 当某一层的文件数 >= `hoodie.timeline.compaction.batch.size` (默认 10) 时,合并该层最老的 N 个文件为下一层的一个文件。
- **源码证据**: `LSMTimelineWriter.compactAndClean()` (L255-270) 实现 Compaction 逻辑
```java
...
**Manifest 原子性保证**:
V2 使用 Manifest + Version 文件来管理 LSM-Tree 的一致性视图,写入新归档文件的流程:
1. 写入 Parquet 数据文件
