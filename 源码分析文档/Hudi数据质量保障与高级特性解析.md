...
| `hoodie.precommit.validators.single.value.sql.queries` | 单值校验 SQL，格式: `query#expectedResult` |
| `hoodie.precommit.validators.failure.policy` | 失败策略: `FAIL`(阻断) 或 `WARN_LOG`(仅告警) |
| `hoodie.precommit.validators.streaming.offset.tolerance.percentage` | 流式偏移校验容忍度百分比 |
...
校验在 `BaseCommitActionExecutor.runPrecommitValidators()` 中被调用，具体逻辑位于
`SparkValidatorUtils.runValidators()`：

```
1. 检查 config.getPreCommitValidators() 是否非空
2. 收集 writeMetadata 中被修改的分区列表 partitionsModified
3. 构建"当前提交后的状态快照" afterState (通过 HoodieTablePreCommitFileSystemView)
4. 构建"当前提交前的状态快照" beforeState (通过 getBaseFileOnlyView)
5. 通过反射实例化所有配置的 Validator 类
6. 用 CompletableFuture 并行运行每个 Validator
7. 收集结果：全部成功则继续，任一失败则抛出 HoodieValidationException
```

**为什么构建"两个快照"？** 这是 Pre-commit Validator 的精髓——在 commit 动作尚未完成时，
通过 `HoodieTablePreCommitFileSystemView` 模拟出"假如 commit 成功后"的文件视图，
与"当前已提交"的文件视图形成对比。Equality Validator 可以断言某些聚合值不变，
...
- 配置格式：`query#expectedResult`，例如 `select count(*) from <TABLE_NAME> where id is null#0`
...
- `OP_KEY_ONLY` 模式只包含 `op` 和 `record_key`
- `DATA_BEFORE` 模式包含 `op`、`record_key`、`before`
- `DATA_BEFORE_AFTER` 模式包含 `op`、`ts_ms`、`before`、`after`
...
2. Hudi 扩展解析器 (HoodieSpark3_5ExtendedSqlAstBuilder) 识别时间旅行子句
3. 生成 TimeTravelRelation 逻辑计划节点
4. ResolveReferences 分析规则匹配 TimeTravelRelation
5. 将 timestamp 转换为 DataSourceReadOptions.TIME_TRAVEL_AS_OF_INSTANT
...
3. 收集目标时间点的所有最新文件列表:
   - 使用 SliceView.getAllLatestFileSlicesBeforeOrOn(instantTime)
...
继承自 `DefaultHoodieRecordPayload`，唯一区别是 `getMetadata()` 返回 `Option.empty()`。
...
**为什么单独一个类？** `DefaultHoodieRecordPayload` 的某些版本会在 metadata 中
追踪事件时间，这会增加序列化/反序列化开销。`EventTimeAvroPayload` 显式跳过这一步，
在高吞吐场景中减少不必要的开销。
