...
**源码位置**：`hudi-client/hudi-client-common/src/main/java/org/apache/hudi/config/HoodieClusteringConfig.java`

```java
public enum LayoutOptimizationStrategy {
    LINEAR,    // 线性排序（单列或多列字典序）
...
public static Dataset<Row> orderDataFrameByMappingValues(
    ...
    if (orderByCols.size() == 1) {
        return df.repartitionByRange(targetPartitionCount, new Column(orderByCols.get(0)));
    }

    // 多列场景：计算 Z-Order / Hilbert 编码值
    // 1. 将每列值映射到 long 范围
    // 2. 根据策略计算空间曲线编码
    // 3. 按编码值排序
    // 4. 输出排序后的 Dataset
...
```java
// 源码位置: SpaceCurveSortingHelper.java, 第 206-234 行
private static byte[] mapColumnValueTo8Bytes(Row row, int index, DataType dataType) {
    if (dataType instanceof LongType) {
        ...
        return BinaryUtil.doubleTo8Byte(...);
    } else if (dataType instanceof StringType) {
        return BinaryUtil.utf8To8Byte(...);  // 取 UTF-8 编码前 8 字节
    }
    // ... 支持 12 种数据类型
}
```
...
private static long mapColumnValueToLong(Row row, int index, DataType dataType) {
    if (dataType instanceof DoubleType) {
        return Double.doubleToLongBits(row.getDouble(index));  // IEEE 754 位模式
    } else if (dataType instanceof StringType) {
        return BinaryUtil.convertStringToLong(row.getString(index));  // UTF-8 前 8 字节转 long
    }
    // ... 同样支持 12 种数据类型
}
```
...
private static JavaRDD<Row> createZCurveSortedRDD(...) {
    return originRDD.map(row -> {
        byte[][] zBytes = fieldMap.entrySet().stream()
            .map(entry -> mapColumnValueTo8Bytes(row, entry.getKey(), entry.getValue().dataType()))
            .toArray(byte[][]::new);
        byte[] zOrdinalBytes = BinaryUtil.interleaving(zBytes, 8);  // ★ 核心：位交错
...
private static JavaRDD<Row> createHilbertSortedRDD(...) {
    return originRDD.mapPartitions(rows -> {
        // ★ 每个分区只创建一个 HilbertCurve 实例，避免重复初始化开销
        ...
        return new Iterator<Row>() {
            ...
            public Row next() {
                ...
                long[] longs = fieldMap.entrySet().stream()
                    .mapToLong(entry -> mapColumnValueToLong(row, entry.getKey(), ...))
                    .toArray();
