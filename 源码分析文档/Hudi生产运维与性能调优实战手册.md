...
| 配置项 | 默认值 | 说明与调优建议 |
|--------|-------|-------------|
| `hoodie.upsert.shuffle.parallelism` | `200` | Upsert 操作的 Shuffle 并行度。计算公式：数据量 / 目标文件大小。100GB 数据、128MB 目标文件大小 = 800。设太小导致 OOM，设太大导致小文件。 |
| `hoodie.insert.shuffle.parallelism` | `200` | Insert 操作并行度，同上公式。建议与 upsert 一致。 |
