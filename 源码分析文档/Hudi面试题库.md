...
**源码证据:**
- COW 写入: `HoodieAbstractMergeHandle` 及其实现类 — 读取旧 Parquet → 合并 → 写新 Parquet
...
**[橙] 问题: COW 表的 HoodieAbstractMergeHandle 和 MOR 表的 HoodieAppendHandle 有什么核心区别？**

**答案:**

**HoodieAbstractMergeHandle 及其实现 (COW):**
```
...
SPARK_FUNCTION_MAP.put("identity", new SparkIdentityFunction(){});
...
SPARK_FUNCTION_MAP.put("from_unixtime", new SparkFromUnixTimeFunction(){});
SPARK_FUNCTION_MAP.put("unix_timestamp", new SparkUnixTimestampFunction(){});
SPARK_FUNCTION_MAP.put("to_date", new SparkToDateFunction(){});
SPARK_FUNCTION_MAP.put("to_timestamp", new SparkToTimestampFunction(){});
SPARK_FUNCTION_MAP.put("date_add", new SparkDateAddFunction(){});
SPARK_FUNCTION_MAP.put("date_sub", new SparkDateSubFunction(){});
...
SPARK_FUNCTION_MAP.put("trim", new SparkTrimFunction(){});
SPARK_FUNCTION_MAP.put("ltrim", new SparkLTrimFunction(){});
SPARK_FUNCTION_MAP.put("rtrim", new SparkRTrimFunction(){});
SPARK_FUNCTION_MAP.put("length", new SparkLengthFunction(){});
SPARK_FUNCTION_MAP.put("regexp_replace", new SparkRegexReplaceFunction(){});
SPARK_FUNCTION_MAP.put("regexp_extract", new SparkRegexExtractFunction(){});
SPARK_FUNCTION_MAP.put("split", new SparkSplitFunction(){});
// 共 22 种内置函数
```
...
| 分区 | 内容 | 记录数量级 | 文件名前缀 |
|------|------|----------|-----------|
| `files` | 文件列表 | 分区数 x 文件数 | `files-` |
| `column_stats` | 列统计 | 文件数 x 列数 | `col-stats-` |
| `bloom_filters` | Bloom Filter | 文件数 | `bloom-filters-` |
| `record_index` | Record Key → FileGroup 映射 | 记录数（可能数十亿） | `record-index-` |
| `expr_index_*` | Expression Index 数据 | 文件数 | `expr-index-` |
| `secondary_index_*` | 二级索引数据 | 变化 | `secondary-index-` |
| `partition_stats` | 分区级统计 | 分区数 | `partition-stats-` |
