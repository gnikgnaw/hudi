...
- 优化：`hoodie.parquet.small.file.limit` 控制小文件合并阈值
...
| `hoodie.upsert.shuffle.parallelism` | `200` | Upsert 并行度 |
| `hoodie.insert.shuffle.parallelism` | `200` | Insert 并行度 |
| `hoodie.bulkinsert.shuffle.parallelism` | `200` | Bulk Insert 并行度 |
| `hoodie.parquet.max.file.size` | `125829120` (120MB) | Parquet 文件最大大小 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `hoodie.index.type` | `SIMPLE` | 索引类型 |
| `hoodie.index.bucket.engine` | `SIMPLE` | Bucket 引擎类型 |
| `hoodie.bucket.index.num.buckets` | `4` | 桶数量 |
...
**3. 小文件合并**
```properties
# 设为 0 可完全关闭小文件合并（Bulk Insert 场景）
